...
fn wasi_c_compiler() -> Option<cc::Build> {
...
fn wasi_cpp_compiler() -> Option<cc::Build> {
...
fn build_wasi_sdk(wasi_sdk_path: &dyn AsRef<Path>) -> PathBuf {
    ...
    let (Some(c_compiler), Some(cpp_compiler)) =
        (wasi_c_compiler(), wasi_cpp_compiler())
    else {
    ...
    cmake_config.out_dir(wasi_build_dir)
        ...
        .init_c_cfg(c_compiler)
        .init_cxx_cfg(cpp_compiler)
